...
async function main() {
  ...
  const deleted = waitFor(['DELETE_COMPLETE'], 30 * 60 * 1000);
  const deleteStatus = deleted?.StackStatus ?? 'UNKNOWN';
  console.log(`DESTROY: final status = ${deleteStatus}`);

  // Prove retention.
  const dbRetained = rdsExists(retained.dbInstance);
  ...
  console.log(`RETENTION CHECK: db=${dbRetained} masterSecret=${secretRetained} urlSecret=${urlSecretRetained} bucket=${bucketRetained}`);
  ...
  // Summarize.
  const ok = status === 'CREATE_COMPLETE' && deleteStatus === 'DELETE_COMPLETE' && dbRetained && secretRetained && urlSecretRetained && bucketRetained;
  console.log(`RESULT: ${ok ? 'COMPOSITE CANARY PASS' : 'COMPOSITE CANARY INCOMPLETE'} (create=${status}, destroy=${deleteStatus}, retained db=${dbRetained}/masterSecret=${secretRetained}/urlSecret=${urlSecretRetained}/bucket=${bucketRetained})`);
  process.exit(ok ? 0 : 1);
}
