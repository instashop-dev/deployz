...
// The Lambdas bundle migration SQL as text via esbuild, which cannot
// glob-import — so every migration must be listed by hand in
// db-connection.ts (shared by the API and worker handlers).
...
const handlerSource = readFileSync(
  fileURLToPath(new URL('../src/lambda/db-connection.ts', import.meta.url)),
  'utf8',
);
...
describe('lambda migration bundling', () => {
