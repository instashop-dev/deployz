 ...
 *                     automated backups (§64). Data lifecycle: RETAIN — a
 *                     stack delete keeps the database, its generated
 *                     credential secrets, and the 7-day automated backups in
 *                     the customer account, reachable with the retained
 *                     credentials. No final snapshot is taken. PURGE deletes
 *                     the retained database and its credential secrets.
 *   5. S3           — versioned application object storage, deployz:-tagged,
 *                     RETAIN on delete; lifecycle rules expire non-current
 *                     versions and abort stale incomplete multipart uploads.
  ...
  constructor(scope: Construct, id: string, props: ApplicationStackProps = {}) {
    ...
    if (databaseRequired) {
      this.databaseSecret = new Secret(this, 'DatabaseSecret', {
        ...
        // Phase 9 data lifecycle (RETAIN): the DB instance outlives the stack
        // (RemovalPolicy.RETAIN below), so its generated credentials must too
        // — otherwise a retained database is unreachable because its password
        // secret was deleted with the stack. PURGE deletes the retained
        // database and its credential secrets together (relay purge sweep).
    ...
    if (databaseRequired) {
      ...
      // Phase 9: scope the database's ingress to the APPLICATION's security
      // group, not the whole VPC CIDR. Express mode is the exception — ECS
      // manages the task security groups there and this template cannot
      // reference them, so Express keeps the whole-VPC-CIDR rule (documented
      // tradeoff). Plain Fargate's service-SG-scoped rule is added below once
      // the service exists.
      if (expressMode) {
      ...
      this.database = new DatabaseInstance(this, 'Database', {
        ...
        // Phase 9 lifecycle decision: RETAIN (documented — do not claim
        // snapshot). Deleting the stack keeps this instance and its automated
        // backups in the account; no final snapshot is taken. The relay's
        // PURGE turns deletion protection off and deletes the instance.
    ...
    if (databaseRequired && databaseUrlEnvNames.length > 0) {
      this.databaseUrlSecret = new Secret(this, 'DatabaseUrlSecret', {
        description:
          ...
          'the password never appears in the template or task definition.',
        // Phase 9: retained WITH the retained database (see DatabaseSecret) —
        // deleting the stack must not destroy the working connection URL to a
        // database that stays behind. PURGE removes it together with the
        // database and the master-credential secret.
        removalPolicy: RemovalPolicy.RETAIN,
...
const dbEnv =
      ...
      // Phase 9: the application may reach the database. A STANDALONE
      // AWS::EC2::SecurityGroupIngress (not an inline rule on the DB SG): an
      // inline SourceSecurityGroupId would make the DB security group depend on
      // the application's security group, which already depends (through the
      // task role's secret grants) on the DatabaseUrlSecret — which resolves
      // this database's endpoint — closing a deploy-time dependency cycle
      // (Database -> DbSecurityGroup -> ServiceSecurityGroup -> task-role
      // policy -> DatabaseUrlSecret -> Database).
      if (dbSecurityGroup !== undefined) {
        const appServiceGroup = fargateService.connections.securityGroups[0];
        if (appServiceGroup !== undefined) {
          new CfnSecurityGroupIngress(this, 'DbAppServiceIngress', {
            groupId: dbSecurityGroup.securityGroupId,
            sourceSecurityGroupId: appServiceGroup.securityGroupId,
            ipProtocol: 'tcp',
            fromPort: DB_PORT,
            toPort: DB_PORT,
      ...
      if (props.workerCommand !== undefined) {
        ...
        // Phase 9: the worker carries the same DATABASE_* env/secrets as the
        // app, so its own security group is scoped in too (standalone ingress
        // resource, same cycle rationale as DbAppServiceIngress above;
        // synth-time branch — no published template provisions a worker).
        const workerGroup = workerService.connections.securityGroups[0];
        if (dbSecurityGroup !== undefined && workerGroup !== undefined) {
          new CfnSecurityGroupIngress(this, 'DbWorkerIngress', {
            groupId: dbSecurityGroup.securityGroupId,
            sourceSecurityGroupId: workerGroup.securityGroupId,
            ipProtocol: 'tcp',
            fromPort: DB_PORT,
            toPort: DB_PORT,
            description: 'Allow the worker to reach RDS PostgreSQL',
          });
        }
      }
    }
