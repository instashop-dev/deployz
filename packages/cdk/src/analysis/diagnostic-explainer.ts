 ...
 *
 * The real gateway is the same injectable `AiGateway` seam + spend-limit config
 * from ai-explainer.ts (PENDING-AI: `createCloudflareAiGateway()` throws until
 * the Vercel AI SDK + Cloudflare credentials are configured).
 */
...
export async function explainDiagnostic(
  ...
  const prompt = truncateToTokens(buildDiagnosticPrompt(failureCode, event), maxTokens);
