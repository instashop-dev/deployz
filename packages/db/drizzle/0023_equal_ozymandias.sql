...
WHERE "state" IN ('REQUESTED', 'QUEUED', 'WAITING', 'RUNNING')
  AND "type" IN ('INSTALL', 'DEPLOY_RELEASE', 'ROLLBACK', 'RESTART', 'DESTROY', 'MIGRATION', 'INFRA_UPGRADE', 'PURGE')
  AND "id" NOT IN (
...
CREATE UNIQUE INDEX "deployment_jobs_one_active_mutating_uidx" ON "deployment_jobs" USING btree ("deployment_id") WHERE "deployment_jobs"."state" IN ('REQUESTED', 'QUEUED', 'WAITING', 'RUNNING') AND "deployment_jobs"."type" IN ('INSTALL', 'DEPLOY_RELEASE', 'ROLLBACK', 'RESTART', 'DESTROY', 'MIGRATION', 'INFRA_UPGRADE', 'PURGE');