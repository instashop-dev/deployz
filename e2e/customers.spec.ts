...
test('Create deployment from the customer page preselects the customer and reuses their row, never creating a second one', async ({
  ...
  // The customer page itself now offers "Create installation" (the invitation
  // flow) rather than linking here directly, but this screen still honors a
  // `?customerId=` preselection — the case this test exercises.
  await page.goto(`/dashboard/deployments/new?customerId=${customer.id}`);
