...
import {
  ...
  DEPLOYMENT_STATES,
  AI_CONFIDENCE_COPY as COPY_MAP_AI_CONFIDENCE_COPY,
  AI_EXPLANATION_SOURCE_NOTE as COPY_MAP_AI_EXPLANATION_SOURCE_NOTE,
...
import {
  ...
  AI_CONFIDENCE_COPY as WEB_AI_CONFIDENCE_COPY,
  AI_EXPLANATION_SOURCE_NOTE as WEB_AI_EXPLANATION_SOURCE_NOTE,
...
describe('§61 failure code vocabulary parity (web ↔ copy-map)', () => {
  ...
  it('explanation fallback copy matches', () => {
    ...
    expect(WEB_AI_CONFIDENCE_COPY).toEqual(COPY_MAP_AI_CONFIDENCE_COPY);
    expect(WEB_AI_EXPLANATION_SOURCE_NOTE).toBe(COPY_MAP_AI_EXPLANATION_SOURCE_NOTE);
  });
