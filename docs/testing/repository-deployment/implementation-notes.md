...
| repo-003 kutt | 4 (2026-09-07, relay with #208/#209/#210) | ENV_BINDING_ERROR → **DEPLOY-013** | The whole chain now runs: zero-task INSTALL, CONFIG_UPDATE SUCCEEDED (secret found and written) — but `unboundSecretKeys: ["JWT_SECRET"]`: the vendor-typed value is write-only and never reached this later install, and the key was not minted because it counted as configured; the deploy's migration one-off exited 1 on the same missing variable (`MIGRATION_FAILED`). Fix PR #211 (mint app-internal secrets). Cleanup through the product |
| repo-003 kutt | 5 (2026-09-07, API with #211) | ENV_BINDING_ERROR → **DEPLOY-005** measured, DEPLOY-013 analyser half | CONFIG_UPDATE minted `JWT_SECRET` (and, wrongly, `DB_PASSWORD`, `REDIS_PASSWORD`, `MAIL_PASSWORD` — the analyser's purpose classifier); the deploy's migration one-off then failed `connect ECONNREFUSED 127.0.0.1:5432`: the app reads `DB_HOST`/`DB_USER`/… and the manifest bound only the standard `DATABASE_*` names, so nothing reached them. Fix PR #212 (the `DB_*` family becomes binding aliases; provisioned and mail credentials are never internal secrets; analysis version 16). Cleanup through the product |
| repo-003 kutt | 6 (2026-09-07, API with #212, analysis v16) | DATABASE_ERROR → **DEPLOY-007** confirmed | The whole configuration chain held: zero-task INSTALL, CONFIG_UPDATE minted `JWT_SECRET` only, the `DB_*` aliases delivered the RDS endpoint (DEPLOY-005 verified) — and the migration one-off, now talking to RDS with `DB_SSL=true`, exited on `Error: self-signed certificate in certificate chain` (`MIGRATION_FAILED`). This reinstates DEPLOY-007 with its log line; the product owner chose option A (PR #213: init container delivers the RDS CA bundle, `NODE_EXTRA_CA_CERTS` + `PGSSLROOTCERT`). Cleanup through the product |
| repo-007 ghostfolio | 1 (2026-09-07, #213 template, relay with #208–#210) | TIMEOUT → **DEPLOY-014** | The whole chain held on a second Node application: zero-task INSTALL, CONFIG_UPDATE minted `ACCESS_TOKEN_SALT` + `JWT_SECRET_KEY`, the migration one-off ran 125 Prisma migrations over TLS behind the RDS CA bundle (DEPLOY-007 verified), the service scaled to 1, HEALTHY / READY, health path 200 over default HTTPS. But the relay never settled the DEPLOY_RELEASE: it read the running digest from the first container of the task — now the `RdsCaBundle` init container — so `currentReleaseId` stayed null, Disconnect was refused 409, the connector could not be removed and the leak audit counted 55 resources. Fix PR #217 (essential container). Operator: service scaled to 0, app stack deleted by hand so the old relay fails the job; `--cleanup --repo repo-007` closes the ledger. Wave stopped (failed cleanup + systemic-bug rule) |
| repo-007 ghostfolio | 2 (2026-09-07, relay with #217) | runtime PASS; harness false-fail on the dependency check; cleanup broken by an operator incident | The full product funnel passed on the fixed relay: zero-task INSTALL, CONFIG_UPDATE, 125 Prisma migrations behind the RDS CA bundle, the release pointer settled 12 min after INSTALL (**DEPLOY-014 verified**), ECS/ALB healthy, default HTTPS ACTIVE, the observation window clean. The harness's dependency check then read the init container's (empty) environment and failed — a harness defect, fixed in PR #221 (`applicationContainerDefinition`, `stoppedExit`). During the product Disconnect, an AWS CLI session on the operator machine (root, a different access key, `aws login` at 17:02:57Z) swept and deleted every IAM role and policy named `deployz-bootstrap-stage-b-*`, including the live relay role, execution role, provisioner policy and permissions boundary of this installation (CloudTrail 17:01–17:04Z). The relay kept polling the control plane but lost all AWS access (no CloudWatch logs, no AWS calls, `stack-exists: false`), the DESTROY job stayed RUNNING and the application stack sat DELETE_FAILED. Not a product defect. Operator recovery: application stack deleted with `--retain-resources Vpc --role-arn <cdk exec role>`, retained RDS (deletion protection off), secrets, bucket, subnet group, security group, subnet and VPC removed by hand, bootstrap stack deleted, then the vendor `disconnect/force-complete` once the relay was DISCONNECTED and the DESTROY stale (60 min). Attempt 3 gives the verdict. Ledger closed by the operator on 2026-09-07 21:43Z: the `--resume` that followed re-ran this ledger's cleanup, the control plane accepted a PURGE for the force-completed deployment (state DELETED, relay DISCONNECTED, cleanupState SKIPPED_RELAY_OFFLINE) and the job sat WAITING for a relay that no longer exists — an observation for the product (a PURGE with no relay to run it can never complete; the API could refuse it or settle it as skipped), not a deployment failure — so the run was stopped and `stageB.cleanupNeeded` cleared by hand after confirming nothing tagged to the installation remained |
| repo-007 ghostfolio | 3 (2026-09-07, relay #217, harness #221) | **PASS** | Third true success. Zero-task INSTALL, CONFIG_UPDATE (minted `ACCESS_TOKEN_SALT`, `JWT_SECRET_KEY`), 125 Prisma migrations behind the RDS CA bundle, release pointer settled, ECS/ALB healthy, default HTTPS ACTIVE, observation window clean, Postgres and Redis bindings verified from the essential container (DEPLOY-005/007/009/014 all exercised on one repository). Disconnect and Purge through the product; the leak audit's one entry was a NAT gateway the tagging API still listed after deletion (the #201 re-check confirmed it gone) |
| repo-021 directus | 1 (2026-09-07, relay #217, harness #221, analysis v16) | ENV_BINDING_ERROR → **DEPLOY-005** second shape | Zero-task INSTALL, CONFIG_UPDATE, scale-up all held; the configured task exited three times on `"DB_HOST" Environment Variable is missing` (CONTAINER_START_FAILED via DEPLOY-011). The analyser's env model had two variables for the repository: directus reads `env['DB_HOST']` through `const env = useEnv()`, invisible to every `process.env`-keyed recogniser — no `DB_*` alias, `KEY`/`SECRET` unminted. Fix PR #224 (analysis v17) merged and deployed; rerun pending. Observation: a template-revision task booted on SQLite for ~5 min beside the configured one. Cleanup through the product |
| repo-039 memos | 1 (2026-09-08, analysis v17) | APPLICATION_ERROR (false success) → **DEPLOY-015**; **DEPLOY-005** third shape | The template revision booted on SQLite and served (ECS/ALB/HTTPS/observation PASS); the configured revision (`MEMOS_DRIVER=postgres`, no `MEMOS_DSN` bound — Go viper env prefix, a name that appears nowhere as a literal) dialled `127.0.0.1:5432` and exited four times; ECS rolled the service back to the template revision (same pinned digest) and the relay reported DEPLOY_RELEASE SUCCEEDED (`alreadyRunning`). The harness caught it only through the observation window's crash count. Disconnect and Purge PASS; the connector removal and leak audit failed on the expired `aws login` (00:15Z) — cleanup rerun pending |
| repo-039 memos | 2 (2026-09-08, relay #225, analysis v18) | INFRA_ERROR → **DEPLOY-016** (control plane down) | Analysis, configuration, build, template, Quick Create and enrolment passed; during INSTALL the control plane began answering 500 to everything — the deploy of PR #228 ran migration 0036, which failed on old duplicate test deployments, and the API Lambda's cached init failure survived in warm containers. Restored by hand (two dead rows retired, Lambda recycled); the run's own funnel never failed. Ledger closed through the product afterwards; attempt 3 gives the verdict |
| repo-039 memos | 3 (2026-09-08, relay #225, analysis v18) | **PASS** | Fourth true success. `MEMOS_DSN` bound as the url alias (the viper shape of DEPLOY-005 fixed), the configured revision came up on Postgres and stayed up, release pointer settled on the revision the deploy rolled out (DEPLOY-015 fix exercised), ECS/ALB healthy, default HTTPS, observation window clean, Postgres and storage bindings verified; Disconnect and Purge through the product; the leak audit's one entry was the tagging API's deleted-NAT lag (re-check confirmed it gone) |
| repo-016 outline | 1 (2026-09-08, analysis v18) | CONFIG_ERROR (ANALYSIS_BUG, a v17 regression) | The control plane's gate demanded nine more keys than the Stage B configuration (APP_NAME, CDN_URL, DD_SERVICE, REDIS_COLLABORATION_URL, REQUEST_TIMEOUT, SERVICES, SMTP_PASSWORD, SMTP_TLS_CIPHERS, AWS_S3_ACCELERATE_URL) — keys outline defaults inside `server/env.ts`. Analysis v17 (PR #224) started counting reads through a module's env object and let them fall into the process.env rules that make a call-argument or bare-secret read a requirement. Fix: analysis v19 records such reads without a requirement (the env module owns the defaults). No AWS resources were created; attempt 2 after the deploy | Systemic-bug rule: ghostfolio, directus and outline need vendor/generated secrets at boot (DEPLOY-009); memos would boot against SQLite until the post-install pass. The product owner chose the generic fix: PR #207 merged (main `b83e6f7`), API deployed (run 34048622229), application templates (Documenso preset, 17 parameters) and the bootstrap template (new relay) republished 2026-09-06 ~17:55Z. Resume: kutt `--repo repo-003 --force`, then `--resume --wave wave-1`, then docuseal and miniflux `--force` reruns |
