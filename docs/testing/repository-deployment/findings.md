...
| DEPLOY-018 | BUILD_ERROR | DEPLOYZ_BUG (build infrastructure) | FIXED — opt-in Docker Hub login (`DOCKERHUB_SECRET_NAME`, `docs/docker-hub-credentials.md`) plus 429 retry and the `build_registry_rate_limited` code; whether production has the variable set is an operator fact | every repository whose Dockerfile pulls a Docker Hub base image, i.e. almost all of them; measured on repo-008 |
