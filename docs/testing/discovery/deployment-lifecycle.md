...
- **Vendor deployment detail**: `apps/web/src/app/dashboard/deployments/[id]/page.tsx` (1443 lines) — uses `useStatusPoll` against `GET /api/deployments/:id`; renders `DeploymentStatusBadge`, `DeploymentProgressCard`, `ActivityFeed` (event log), `InfrastructureEvents` (stack-events timeline, gated by `stage`), `InfrastructureSection` (resource inventory, PR #66).
