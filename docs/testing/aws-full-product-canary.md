...
5. **Healthy**: the deployment reaches HEALTHY only after a heartbeat with
   ...
   is plain HTTP until a custom domain exists, and HTTPS-First browsers
   refuse http). The vendor detail shows *Open app*, the URL, and
   *Setting up HTTPS* waiting for a custom domain.
