...
- **Jev is not OpenAI-compatible.** Jev (TypeSafe "System One") uses `POST /v1/systemone` with typed questions. The existing `/compat` unified endpoint cannot carry it. Route through a Cloudflare AI Gateway **custom provider** (slug `typesafe`, base URL `https://api.typesafe.ai`): requests go to `{gateway}/custom-typesafe/v1/systemone`. This keeps the "no direct Jev path" rule. Requires one-time custom-provider registration in the Cloudflare account plus a TypeSafe API key — external setup needed only for live phases; PR 1–3 tests use fixtures.
