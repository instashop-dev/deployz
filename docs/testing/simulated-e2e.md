...
| `purge-failure` | Same clean destroy as `retained-resources`, then PURGE's orphan sweep finds one tag-owned S3 bucket it cannot delete (`deleteBucket` always fails) | `state: DELETED` (never resurrected); `cleanupState: PURGE_FAILED` (never `COMPLETE`) | Never claims a clean purge — the retained-resources warning stays | `purge.failed` event recorded with the leftover bucket name in `payload.error`; no `purge.completed` event | `e2e/scenario-lifecycle.spec.ts` |
