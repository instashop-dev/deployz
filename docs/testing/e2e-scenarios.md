...
| `update-failure` | Install reaches HEALTHY; a `v1` release deploys and succeeds; a `v2` release rollout trips the ECS deployment circuit breaker | `state: FAILED`; `failure.code: ECS_DEPLOYMENT_FAILED`; `currentReleaseId` stays `v1` — never advances past the last release that actually deployed | Fleet shows Failed, with `v1` still the current release | Event log contains `deploy.failed` | `e2e/scenario-lifecycle.spec.ts` |
...
| `rollback-failure` | Same as `update-failure`, but the rollback to `v1` also fails | `state: FAILED`; `failure.code: ECS_DEPLOYMENT_FAILED`; release pointers unchanged — never a false success | Still Failed | Event log contains `rollback.failed`, never `rollback.completed` | `e2e/scenario-lifecycle.spec.ts` |
