...
| `regional-https-ra` | First deployment for a customer whose relay reports the customer's `dnsScope`: one `ENSURE_CERTIFICATE` request for `*.c-<scope>.deployz-fixture.test`, the scope validation CNAME written once (unproxied), INSTALL completes, the scoped routing CNAME `d-<id>.c-<scope>.…` written (unproxied), the machine reaches ACTIVE | `state: HEALTHY`, `stage: READY`, `appUrl: https://d-<id>.c-<scope>.deployz-fixture.test` | "Preparing secure access" with all three sub-steps done | Exactly one certificate in the simulated ACM registry; `httpsProgress.substeps` all `done` | `e2e/scenario-regional-https.spec.ts` |
| `regional-https-rb` | Second deployment for the same customer, account and region after the first is READY | READY behind the same certificate ARN | Sub-steps complete quickly | No new ENSURE request in the registry; INSTALL payload carried `regionalCertificateArn`; same ARN on both deployments | `e2e/scenario-regional-https.spec.ts` |
| `regional-https-rc` | A deployment for the same customer in another region | READY | Same `c-<scope>` namespace | A second certificate for the same domain in the other region; the namespace label is unchanged | `e2e/scenario-regional-https.spec.ts` |
| `regional-https-rd` | DESTROY one of two sibling deployments, PURGE it, then DESTROY + PURGE the last one | First: `DELETED`, certificate retained; last: `DELETED`, certificate removed | Removed | DESTROY removes only the scoped routing record; a PURGE with a sibling alive carries no `regionalCertificates`; the last PURGE carries them and removes the certificate and its validation record | `e2e/scenario-regional-https.spec.ts` |
| `regional-https-re` | The pending certificate is deleted from the simulated ACM registry before issuance | READY | Sub-steps progress again | A second ENSURE request replaces the ARN on the same row; one row; no duplicate | `e2e/scenario-regional-https.spec.ts` |
| `regional-https-rf` | The registry answers `FAILED` (`CAA_ERROR`) for the scope | `state: HEALTHY` (never FAILED); certificate row `ERROR` | Sub-steps in the attention state, no spinner; retry available | `httpsProgress.state: FAILED`; `POST default-https/retry` resets the row and the next cycle issues the certificate | `e2e/scenario-regional-https.spec.ts` |
