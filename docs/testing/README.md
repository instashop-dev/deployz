...
| [`../ai-analysis.md`](../ai-analysis.md) | The AI analysis / preflight / diagnosis reference: how to run the analysis tests, the evaluation corpus (`packages/analysis/test/eval-corpus.test.ts`), how to add fixtures, compatibility rules and failure signatures, and how to test AI changes without wording assertions |
...
| [`repository-deployment/README.md`](repository-deployment/README.md) | The Stage B repository-deployment audit: the same 100-repository corpus taken through the real production path (CodeBuild, ECR, relay, CloudFormation, ECS, ALB, RDS/ElastiCache/S3, default HTTPS, Disconnect/Purge) into the test AWS account, one result per Stage A id, findings DEPLOY-nnn, cleanup and leak-audit rules (`pnpm benchmark:deploy`) — real AWS, opt-in |
