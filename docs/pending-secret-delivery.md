...
Both use the same cipher and the same key. The `deployz_generated`
environment variables are minted by the relay inside the customer account
and never enter the control plane; the template's secret *parameters* are
generated by the API per INSTALL and ride the job payload until claim (see
*Accepted plaintext paths*). Customer values typed once the relay is
connected are delivered directly (same section).
