...
full simulated suite (`pnpm e2e:scenarios`) → real AWS only as escalation
(the version canary: `pnpm e2e:canary:versions profile --profile stateless`,
then `core`). Use `pnpm test:affected` and `pnpm test:escalation` for guided
selection. Full policy at `docs/testing/strategy.md`.
